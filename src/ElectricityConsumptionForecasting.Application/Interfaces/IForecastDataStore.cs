...
public interface IForecastDataStore
{
    ...
    Task<IReadOnlyList<CustomerProfile>> GetCandidateProfilesAsync(CustomerProfile targetProfile, int maxCandidates, CancellationToken cancellationToken);
    ...
    Task<IReadOnlyDictionary<string, CustomerMonthlyConsumption>> GetActualConsumptionForCustomersAsync(IEnumerable<string> billIdentifiers, int year, int month, CancellationToken cancellationToken);
